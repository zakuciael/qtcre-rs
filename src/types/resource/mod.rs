...
impl<'a> Resource<'a> {
  pub(crate) fn derive(index: u32, root: &'a ResourceReader<'a>) -> error::Result<Resource<'a>> {
    let ptr = root.find_ptr(index);
    let flags = root
      .bytes
      .read_from_offset::<u16>(ptr + mem::size_of::<u32>())
      .wrap_error_lazy(|| format!("Failed to read resource flags at {}", to_hex!(ptr)))?;

    Ok(if flags & ResourceFlags::Directory as u16 > 0 {
      Resource::Directory(ResourceDirectory::new(index, root))
    } else {
      Resource::File(ResourceFile::new(index, root))
    })
  }
